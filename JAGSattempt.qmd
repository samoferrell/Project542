...
ggplot(boxplot_no_outliers, aes(x = Region, y = area_burned, fill = Region)) +
  geom_boxplot() +
  facet_wrap(~ year) +
    labs(title = "Boxplots of Area Burned across Regions and Years (outliers removed)",
       y = "Area Burned",
...
ggplot(boxplot_no_outliers, aes(x = month, y = area_burned, fill = month)) +
  geom_boxplot() +
  labs(title = "Boxplots of Area Burned across Months (outliers removed)",
       y = "Area Burned",
       x = "Month",
       caption = "Outliers Removed for Visualization Purposes")
...
DIC1    <- dic.samples(model,n.iter=1000,n.thin = 5, progress.bar="none")
DIC1
```
